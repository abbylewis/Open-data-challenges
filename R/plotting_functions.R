...
plot_facet_result <- function(df = survey_results,
  ...
  # plot results
  if (as_percent) {
    p <- ggplot(df_plot, aes(x = if(switch_x_y) percentage else get(column_id),
                             y = if(switch_x_y) get(column_id) else percentage)) +
      geom_bar(stat = 'identity') +
      labs(x = if(switch_x_y) 'Percentage of respondants' else column_id,
           y = if(switch_x_y) column_id else 'Percentage of respondants') +
      facet_wrap(~get(group_id))
  } else {
    p <- ggplot(df_plot, aes(x = if(switch_x_y) count else get(column_id),
                             y = if(switch_x_y) get(column_id) else count)) +
      geom_bar(stat = 'identity') +
      labs(x = if(switch_x_y) 'Number of respondants' else column_id,
           y = if(switch_x_y) column_id else 'Number of respondants') +
      facet_wrap(~get(group_id))
  }
  ...
  p +
    theme_bw() +
    labs(subtitle = questions[1, column_n]) +
    theme(axis.text.x = element_text(angle = if(switch_x_y) 0 else 90))
}
